...
- ADD **Self-bot guard on Maint and Gear.** With `.playerbots bot self` on, your own character obeys `maintenance` and `autogear` said in party chat, exactly like a bot. `maintenance` permanently learns weapon skills, professions and secondary skills (First Aid, Fishing, Cooking), every available spell, spends all talent points, adds glyphs, enchants and gems gear, and fills your bags. MiniGM now watches for the server's `Enable player botAI` / `Disable player botAI` lines, and while self-bot is on either button opens a warning that lists every change and sends nothing until you type `Accept` (case sensitive). The list was checked line by line against `MaintenanceAction::Execute` on mod-playerbots `b6696bd`.
- CHANGE **Maint / Gear split into two buttons: Maint and Gear.** Gear sends only `autogear` (equipment, nothing else), so you can gear a character without `maintenance` also teaching it skills, professions and talents. Maint sends `maintenance` + `nc -loot`. The panel is 25 px taller.
- ADD **Maint or Gear on yourself.** Solo, each button offers its own command on your character, behind a warning window that lists only what that command changes, and sends nothing until you type `Accept` (case sensitive). MiniGM turns self-bot on, whispers you the one command, waits for the bot AI's own answer ("I'm auto gearing", "I'm maintaining", or an error; up to 30 s), and only then turns self-bot back off (only if it turned it on). A fixed 5 s wait was tried first and switched self-bot off before autogear ran. If the server refuses self-bot, nothing else is sent. In a group: `/mgm maintme` / `/mgm gearme`.
- CHANGE The warning is a custom 480 px window - one item per line, left-aligned, opaque, above nameplates, movable, drag the bottom-right corner to scale it (size and position remembered, `/mgm reset` restores), Esc cancels - instead of a 320 px StaticPopup that wrapped every line.
- CHANGE **One confirm window for everything.** The Modify Char "type Accept" StaticPopup is gone; level and gold changes on yourself now open the same 480 px confirm window as Maint and Gear, showing the exact change (`Parriahtest: add 10g 0s 0c`). One window, one word, everywhere.
- FIX **A wrong confirm word no longer fails silently.** The window stays open, shows "That was not  Accept - nothing sent. Try again, or Cancel." in red inside the window, clears the box and refocuses it. Before, the only feedback was a chat line behind the dialog and the window closed, which read as the button doing nothing.
- FIX Enter works everywhere a click does: the warning window takes Enter (Send) and Esc (Cancel) even when the text box has lost focus, the teleport confirm takes Enter, and Enter in the teleport search box closes the keyboard focus.
...
- CHANGE Modify Char's "modify YOUR OWN character" prompt now states the exact change before you type Accept, e.g. `Parriahtest: level 1 -> 10` or `Parriahtest: add 500g 0s 0c`.
